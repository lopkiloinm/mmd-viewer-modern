...
const GTAOBlendShader = {
	...
	fragmentShader: /* glsl */`
		...
		void main() {
			...
			// Multiply-blend against the beauty pass: raw AO texels can go very dark and
			// flatten mid-tones (reads as “everything got graded down”). Clamp the minimum
			// multiplier so occlusion stays local/contact-heavy rather than a global dim.
			vec3 occ = mix( vec3( 1. ), texel.rgb, intensity );
			occ = max( occ, vec3( 0.52 ) );
			gl_FragColor = vec4( occ, texel.a );
		}`
