/**
 * Runtime: vendored three.js r164 `examples/jsm` subset in this folder.
 * Types: re-export from @types/three so we do not duplicate .d.ts trees.
 */
declare module '@three-jsm/environments/RoomEnvironment.js' {
  export { RoomEnvironment } from 'three/examples/jsm/environments/RoomEnvironment.js';
}
declare module '@three-jsm/controls/OrbitControls.js' {
  export { OrbitControls } from 'three/examples/jsm/controls/OrbitControls.js';
}

declare module '@three-jsm/postprocessing/EffectComposer.js' {
  export { EffectComposer } from 'three/examples/jsm/postprocessing/EffectComposer.js';
}
declare module '@three-jsm/postprocessing/RenderPass.js' {
  export { RenderPass } from 'three/examples/jsm/postprocessing/RenderPass.js';
}
declare module '@three-jsm/postprocessing/ShaderPass.js' {
  export { ShaderPass } from 'three/examples/jsm/postprocessing/ShaderPass.js';
}
declare module '@three-jsm/postprocessing/UnrealBloomPass.js' {
  export { UnrealBloomPass } from 'three/examples/jsm/postprocessing/UnrealBloomPass.js';
}
declare module '@three-jsm/postprocessing/BokehPass.js' {
  export { BokehPass } from 'three/examples/jsm/postprocessing/BokehPass.js';
}
declare module '@three-jsm/postprocessing/GTAOPass.js' {
  export { GTAOPass } from 'three/examples/jsm/postprocessing/GTAOPass.js';
}
declare module '@three-jsm/postprocessing/AfterimagePass.js' {
  export { AfterimagePass } from 'three/examples/jsm/postprocessing/AfterimagePass.js';
}
declare module '@three-jsm/postprocessing/GlitchPass.js' {
  export { GlitchPass } from 'three/examples/jsm/postprocessing/GlitchPass.js';
}
declare module '@three-jsm/postprocessing/OutputPass.js' {
  export { OutputPass } from 'three/examples/jsm/postprocessing/OutputPass.js';
}

declare module '@three-jsm/shaders/HueSaturationShader.js' {
  export { HueSaturationShader } from 'three/examples/jsm/shaders/HueSaturationShader.js';
}
declare module '@three-jsm/shaders/VignetteShader.js' {
  export { VignetteShader } from 'three/examples/jsm/shaders/VignetteShader.js';
}
declare module '@three-jsm/shaders/BrightnessContrastShader.js' {
  export { BrightnessContrastShader } from 'three/examples/jsm/shaders/BrightnessContrastShader.js';
}
declare module '@three-jsm/shaders/SepiaShader.js' {
  export { SepiaShader } from 'three/examples/jsm/shaders/SepiaShader.js';
}

declare module '@three-jsm/loaders/OBJLoader.js' {
  export { OBJLoader } from 'three/examples/jsm/loaders/OBJLoader.js';
}
declare module '@three-jsm/loaders/MTLLoader.js' {
  export { MTLLoader } from 'three/examples/jsm/loaders/MTLLoader.js';
}
declare module '@three-jsm/loaders/FBXLoader.js' {
  export { FBXLoader } from 'three/examples/jsm/loaders/FBXLoader.js';
}
declare module '@three-jsm/loaders/GLTFLoader.js' {
  export { GLTFLoader } from 'three/examples/jsm/loaders/GLTFLoader.js';
}
declare module '@three-jsm/loaders/ColladaLoader.js' {
  export { ColladaLoader } from 'three/examples/jsm/loaders/ColladaLoader.js';
}
declare module '@three-jsm/loaders/STLLoader.js' {
  export { STLLoader } from 'three/examples/jsm/loaders/STLLoader.js';
}
declare module '@three-jsm/loaders/PLYLoader.js' {
  export { PLYLoader } from 'three/examples/jsm/loaders/PLYLoader.js';
}
declare module '@three-jsm/loaders/VRMLLoader.js' {
  export { VRMLLoader } from 'three/examples/jsm/loaders/VRMLLoader.js';
}
